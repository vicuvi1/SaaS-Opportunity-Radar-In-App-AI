/**
 * Discover v4 Eval — Lean prompt + claude-sonnet-4-6
 * Run: npx tsx eval-discover-v4.ts
 *
 * Single cell: lean + claude-sonnet-4-6
 * 20 cases: 10 profile-only (no instruction), 10 same profiles + instruction
 * Output: eval-discover-v4-results.md
 */
...
async function main() {
  console.log(`Discover v4 eval: lean + claude-sonnet-4-6 | ${TEST_CASES.length} cases (${TEST_CASES.filter(t => t.group === "no-instruction").length} no-instruction, ${TEST_CASES.filter(t => t.group === "with-instruction").length} with-instruction)\n`);
  ...
  for (let i = 0; i < TEST_CASES.length; i += CONCURRENCY) {
    ...
    for (const r of batchResults) {
      const ideas = r.output?.opportunityZones?.flatMap(z => z.ideas ?? []).length ?? 0;
      const status = r.error ? `FAILED: ${r.error.slice(0, 80)}` : `OK (${(r.durationMs / 1000).toFixed(1)}s, ${ideas} ideas)`;
  ...
  for (let i = 0; i < profileIds.length; i++) {
    ...
    const noInstrR = allResults.find(r => r.id === noInstrIds[i]);
    const withInstrR = allResults.find(r => r.id === withInstrIds[i]);
  ...
  fs.writeFileSync(path.join(__dirname, "../results/discover-v4.md"), report, "utf-8");
  console.log(`\nReport written to test/results/discover-v4.md`);
}
